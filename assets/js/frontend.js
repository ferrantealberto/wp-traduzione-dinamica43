...
(function($) {
    ...
    let isLanguageChanging = false; // Flag per evitare doppi cambi
    let liveTranslationEnabled = true; // Abilita traduzione live di default
    let streamingEnabled = true; // Abilita streaming per traduzioni più veloci
    ...
    /**
     * Traduce contenuto pagina con selettori migliorati e supporto live
     */
    function translatePageContent(targetLang) {
        console.log('DPT: Avvio traduzione contenuto pagina per lingua:', targetLang);
        
        // Usa traduzione live se abilitata
        if (liveTranslationEnabled) {
            console.log('DPT: Usando traduzione live per performance migliori');
            translatePageContentLive(targetLang);
            return;
        }
        
        // Selettori ampi per catturare tutto il contenuto
    ...
    /**
     * NUOVO: Traduzione live con streaming per performance migliori
     */
    function translatePageContentLive(targetLang) {
        console.log('DPT: Avvio traduzione live per lingua:', targetLang);
        
        // Selettori ottimizzati per traduzione live
        const textElements = $('h1, h2, h3, h4, h5, h6, p, li, td, th, div.content, div.description, article')
            .not('.dpt-language-switcher, .dpt-flag-switcher, .dpt-popup-overlay, .dpt-sidebar-panel, .dpt-translation-indicator, .dpt-success-notification, .dpt-error-notification, .dpt-no-translate')
            .filter(function() {
                const $this = $(this);
                const text = $this.text().trim();
                
                // Filtri ottimizzati per traduzione live
                return text.length > 10 && 
                       text.length < 1000 && 
                       $this.children().length < 5 && // Permette alcuni elementi figli
                       !/^\d+$/.test(text) && // Non solo numeri
                       !$this.is('script, style, noscript') && // Non script/style
                       !$this.hasClass('dpt-no-translate'); // Non elementi marcati
            })
            .slice(0, 100); // Aumentato limite per traduzione live
        
        let elementsToTranslate = 0;
        let elementsTranslated = 0;
        
        console.log('DPT: Elementi candidati trovati per traduzione live:', textElements.length);
        
        // Raggruppa elementi per batch per traduzione più veloce
        const batchSize = 5;
        const batches = [];
        let currentBatch = [];
        ...
        textElements.each(function() {
            ...
            if (text.length > 10) {
                ...
                if (currentBatchLength + text.length > 1000 || currentBatch.length >= batchSize) {
                    // Batch pieno, inizia nuovo batch
        ...
        console.log('DPT: Creati', batches.length, 'batch per traduzione live');
        ...
        // Aggiorna indicatore con progresso
        showTranslationIndicator(`Traduzione live di ${elementsToTranslate} elementi...`);
        
        // Traduci batch in sequenza
        ...
        function processBatch(index) {
            if (index >= batches.length) {
                ...
                hideLoadingState();
                hideTranslationIndicator();
                showSuccess('Traduzione live completata (' + elementsTranslated + ' elementi)');
                console.log('DPT: Traduzione live completata');
                return;
            }
            
            const batch = batches[index];
            console.log('DPT: Processo batch', index + 1, 'di', batches.length, 'con', batch.length, 'elementi');
            
            // Prepara contenuti per traduzione batch
            const contents = batch.map(item => item.content);
            const batchText = contents.join('\n---SPLIT---\n');
            
            // Usa streaming se abilitato
            if (streamingEnabled && batchText.length > 500) {
                translateBatchStreaming(batch, batchText, targetLang, function() {
                    batchesProcessed++;
                    updateProgress(batchesProcessed, batches.length);
                    processBatch(index + 1);
                });
            } else {
                translateBatch(batch, batchText, targetLang, function() {
                    batchesProcessed++;
                    updateProgress(batchesProcessed, batches.length);
                    processBatch(index + 1);
                });
        ...
        function updateProgress(processed, total) {
            ...
            showTranslationIndicator(`Traduzione live: ${percent}% completata...`);
        }
        
        // Avvia processo batch
        processBatch(0);
    }
    
    /**
     * NUOVO: Traduzione batch standard
     */
    function translateBatch(batch, batchText, targetLang, callback) {
        $.ajax({
            url: dptFrontend.ajaxUrl,
            type: 'POST',
            data: {
                action: 'dpt_batch_translate',
                content: batchText,
                source_lang: dptFrontend.defaultLang,
                target_lang: targetLang,
                nonce: dptFrontend.nonce
            ...
            success: function(response) {
                if (response.success && response.data.translations) {
                    const translations = response.data.translations.split('\n---SPLIT---\n');
                    
                    // Applica traduzioni agli elementi
                    batch.forEach(function(item, index) {
                        if (translations[index]) {
                            $(item.element).text(translations[index]);
                            $(item.element).removeClass('dpt-translating');
                            elementsTranslated++;
                        }
                    });
                }
            ...
            error: function(xhr, status, error) {
                console.warn('DPT: Errore traduzione batch:', error);
                
                // Rimuovi classe translating dagli elementi
                batch.forEach(function(item) {
                    $(item.element).removeClass('dpt-translating');
                });
            },
    ...
    /**
     * NUOVO: Traduzione batch con streaming per contenuti lunghi
     */
    function translateBatchStreaming(batch, batchText, targetLang, callback) {
        const eventSource = new EventSource(dptFrontend.ajaxUrl + 
            '?action=dpt_live_translate' + 
            '&content=' + encodeURIComponent(batchText) + 
            '&source_lang=' + dptFrontend.defaultLang + 
            '&target_lang=' + targetLang + 
            '&streaming=true' + 
            '&nonce=' + dptFrontend.nonce);
        ...
        let timeoutId = setTimeout(function() {
            console.warn('DPT: Timeout streaming, chiudo connessione');
            eventSource.close();
            if (callback) callback();
        }, 30000);
        
        eventSource.addEventListener('translation_start', function(e) {
            console.log('DPT: Streaming traduzione iniziato');
        });
        
        eventSource.addEventListener('translation_progress', function(e) {
            const data = JSON.parse(e.data);
            console.log('DPT: Progresso streaming:', data.progress + '%');
            
            // Aggiorna indicatore
            showTranslationIndicator(`Traduzione streaming: ${data.progress}%...`);
        ...
        eventSource.addEventListener('translation_complete', function(e) {
            const data = JSON.parse(e.data);
            console.log('DPT: Streaming traduzione completato');
        ...
        eventSource.onerror = function(e) {
            console.error('DPT: Errore streaming:', e);
            clearTimeout(timeoutId);
            eventSource.close();
            
            // Fallback a traduzione standard
            console.log('DPT: Fallback a traduzione standard');
            translateBatch(batch, batchText, targetLang, callback);
        };
    }
    ...
    window.DynamicTranslator = {
        ...
        // Configurazione live translation
        setLiveTranslation: function(enabled) {
            liveTranslationEnabled = enabled;
            console.log('DPT: Live translation ' + (enabled ? 'abilitata' : 'disabilitata'));
        },
        setStreamingEnabled: function(enabled) {
            streamingEnabled = enabled;
            console.log('DPT: Streaming ' + (enabled ? 'abilitato' : 'disabilitato'));
        },
        // Funzioni di debug
